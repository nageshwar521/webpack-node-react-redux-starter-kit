...
const NavItemCollapsed = (props) => {
  ...
  const hasChildrenAndIsActive =
    hasChildren && itemsAll.filter((item) => `#${item.link}` === window.location.hash).length > 0;
  ...
  const ListItemRoot = (
    ...
    >
      ...
      >
        {!!ListItemIconInner && (
          <ListItemIcon style={IconStyles} className={clsx(classes.navItemIcon, IconClassName)}>
            {ListItemIconInner}
          </ListItemIcon>
        )}
